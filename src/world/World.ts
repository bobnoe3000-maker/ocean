...
export class World {
  ...
  apply(spec: SceneSpec): void {
    ...
    if (this.reflection) this.ocean.uniforms.uReflF.value = (spec.style === 'stylized' ? 0.22 : 1) * (1 - 0.85 * this.lighting.night); // stylised: the mirror is a whisper; the warm horizon reflected into the near water read as a causeless band // stylised: mirror images stay soft so hull and dock reflections never read as second shadows
