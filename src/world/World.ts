...
export class World {
  ...
  apply(spec: SceneSpec): void {
    ...
    { const fwd = new THREE.Vector3(); this.camera.getWorldDirection(fwd); const k = THREE.MathUtils.smoothstep(fwd.dot(W.uSunDir.value), 0.55, 0.9) * Math.max(0, 1 - this.lighting.night); this.renderer.toneMappingExposure = this.lighting.exposure * (1 - 0.78 * k); }
    this.post.setStyle(spec.style === 'stylized');
    ...
    { const [m1x, m1z] = vistaToWorld(LAYOUT.moleFrom[0], LAYOUT.moleFrom[1]); const [m2x, m2z] = vistaToWorld(LAYOUT.moleTo[0], LAYOUT.moleTo[1]); this.ocean.setMole(m1x, m1z, m2x, m2z, 5.5); }
    ...
    if (this.reflection) this.ocean.uniforms.uReflF.value = (spec.style === 'stylized' ? 0.12 : 1) * (1 - 0.85 * this.lighting.night); // stylised: the mirror is a whisper; the warm horizon reflected into the near water read as a causeless band // stylised: mirror images stay soft so hull and dock reflections never read as second shadows
