...
float hfield(sampler2D t, vec2 uv) { return texture2D(t, clamp(uv, 0.0, 1.0)).r; } // hardware bilinear (R16F is filterable on GLES3)
      ...
      replace: [
        ['#include <normal_fragment_maps>', /* glsl */ `
          ...
          float hTex = hfield(tHeight, (P.xz - uGrid.xy) / uGrid.z + 0.5);
          ...
          if (sandW > 0.004) {
            ...
            float dry = smoothstep(-0.3, 0.6, h); // the drifts belong to dry sand; under water they read as blotches through the turquoise
            sS.a *= mix(vec3(1.0), mix(vec3(0.88, 0.85, 0.8), vec3(1.04, 1.03, 1.0), smoothstep(0.3, 0.7, nz2.g)), dry);
            sS.a *= mix(vec3(1.0), mix(vec3(1.0, 0.97, 0.9), vec3(0.96, 0.98, 1.0), smoothstep(0.4, 0.6, nz3.r)), dry);
          ...
          // so the terrain mesh's cut through the water plane is invisible (both sides carry the same collar)
          {
            ...
            float washEdge = 0.14 + (lace - 0.5) * 0.2 + 0.06 * sin(uTime * 0.7 + P.x * 0.05 + P.z * 0.03); // a wet rim, not a chalk sheet
            float wash = (1.0 - smoothstep(washEdge - 0.08, washEdge + 0.08, h)) * smoothstep(-0.6, -0.15, h) * sandW;
